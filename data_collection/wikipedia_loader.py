...
from qdrant_client.models import (
    PointStruct,
)  # Import the PointStruct to store the vector and payload
...
def upsert_wikipedia_to_db():
    try:
        ...
        article_df = pd.read_csv(
            "../data/wikipedia.csv", usecols=["url", "title", "text"]
        )
        ...
        for k, v in tqdm(
            article_df.iterrows(), desc="Upserting articles", total=len(article_df)
        ):
            try:
                client.upsert(
                    collection_name="wikipedia",
